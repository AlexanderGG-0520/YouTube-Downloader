...
Git がインストールされていないか、PATH が反映されていません。

Chocolatey を使う場合は、管理者 PowerShell で次を実行してください。
...
### このフォルダはGitリポジトリではありません、と出る

`update.bat` を実行しているフォルダに `.git` フォルダがありません。

GitHub から取得したリポジトリのフォルダ内で `update.bat` を実行してください。

### ローカル変更があって pull できない

手元で編集したファイルがある場合、`git pull origin main` が失敗することがあります。

このツールはローカル変更を勝手に破棄しません。表示された Git のエラー内容を確認し、必要な変更を保存または整理してから、もう一度 `update.bat` を実行してください。
